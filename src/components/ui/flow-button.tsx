...
export function FlowButton({
  ...
  const shell = cn(
    ...
    "whitespace-nowrap border-[1.5px] border-border bg-transparent font-semibold text-foreground",
  ...
  // Ortadan açılan dolgu. Her iki varyantta da gerekli: yazı hover'da arka
  // plan rengine döndüğü için arkasında bu dolgu yoksa görünmez oluyor.
  const fill = (
    <span
      aria-hidden
      className={cn(
        ...
        // Daire, en geniş etiketi bile örtecek kadar büyümeli.
        isSmall
          ? "group-hover:h-[360px] group-hover:w-[360px]"
          : "group-hover:h-[460px] group-hover:w-[460px]",
      )}
    />
  );
    ...
    <>
      ...
      <ArrowRight
        ...
        className={cn(
          arrowBase,
          isSmall
            ? "h-3.5 w-3.5 left-[-25%] group-hover:left-3"
            : "h-4 w-4 left-[-25%] group-hover:left-4",
        )}
      />
